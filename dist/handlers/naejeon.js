...
function ephemeral(content) {
    return { content, flags: discord_js_1.MessageFlags.Ephemeral };
}
async function updateSessionMessage(session, guild) {
    ...
    if (!message)
        return;
    const payload = await (0, ui_1.buildMessagePayload)(session, guild);
    await message.edit(payload);
}
async function refresh(interaction, session, guild) {
    await interaction.deferUpdate();
    (0, session_store_1.saveSession)(session);
    await updateSessionMessage(session, guild);
}
async function terminateSession(interaction, session, guild, state) {
    await interaction.deferUpdate();
    session.state = state;
    await updateSessionMessage(session, guild);
    (0, session_store_1.deleteSession)(session.id);
}
async function continueAsNewMessage(interaction, session, guild, oldMessageNote) {
    const channel = guild?.channels.cache.get(session.channelId);
    if (!channel) {
...
async function startRematchAsNewMessage(interaction, session, guild) {
    (0, types_1.resetForRematch)(session);
    await continueAsNewMessage(interaction, session, guild, "**재경기**가 아래에서 시작되었습니다.");
}
...
function canActAsHost(session, userId, interaction) {
    return userId === session.hostId || isServerAdmin(interaction);
}
function canActForCurrentCaptain(session, userId, interaction) {
    const currentCaptain = session.pickOrder[session.currentPickerIndex];
    return (userId === currentCaptain ||
...
function startDraft(session) {
    ...
    session.teams = { [c1]: [], [c2]: [] };
    session.remaining = session.participants.filter((id) => !session.captains.includes(id));
}
function advanceAfterPick(session) {
    session.draftSelections = [];
...
function expelParticipants(session, userIds) {
    session.participants = session.participants.filter((id) => !userIds.includes(id));
    session.captainCandidates = session.captainCandidates.filter((id) => !userIds.includes(id));
    session.kickSelections = session.kickSelections.filter((id) => !userIds.includes(id));
}
async function handleNaejeonCommand(interaction) {
    ...
    const existing = (0, session_store_1.getActiveSessionByChannel)(interaction.channelId);
    ...
    if (existing && ["complete", "cancelled", "ended"].includes(existing.state)) {
        (0, session_store_1.deleteSession)(existing.id);
    }
    await interaction.deferReply();
    const sessionId = (0, session_store_1.createSessionId)();
    const session = (0, types_1.createSession)(sessionId, interaction.channelId, "", interaction.user.id);
    const payload = await (0, ui_1.buildMessagePayload)(session, interaction.guild);
...
function canModifyParticipants(session) {
...
async function handleAddParticipantCommand(interaction) {
    ...
    if (!canModifyParticipants(session)) {
        await interaction.reply({
...
async function handleRemoveParticipantCommand(interaction) {
    ...
    if (!canModifyParticipants(session)) {
        await interaction.reply({
            content: "참가자 제거는 **모집** 또는 **팀장 선정** 단계에서만 가능합니다.",
            flags: discord_js_1.MessageFlags.Ephemeral,
        });
        return;
    }
    ...
    if (!canActAsHost(session, userId, interaction)) {
        await interaction.reply({
            content: `참가자 제거는 <@${session.hostId}> 또는 **서버 관리자**만 할 수 있습니다.`,
            flags: discord_js_1.MessageFlags.Ephemeral,
        });
        return;
    }
    ...
    if (!session.participants.includes(target.id)) {
        await interaction.reply({
            content: `<@${target.id}>님은 참가자가 아닙니다.`,
            flags: discord_js_1.MessageFlags.Ephemeral,
        });
        return;
    }
    expelParticipants(session, [target.id]);
    (0, session_store_1.saveSession)(session);
    await updateSessionMessage(session, interaction.guild);
    await interaction.reply({
        content: `<@${target.id}>님을 참가자에서 제거했습니다. (현재 ${session.participants.length}명)`,
        flags: discord_js_1.MessageFlags.Ephemeral,
    });
}
async function handleNaejeonButton(interaction) {
    ...
    if (!session) {
        await interaction.reply({
            content: "내전 세션이 만료되었습니다. 봇이 재시작되었거나 **오래된 메시지**일 수 있습니다.\n" +
    ...
    const userId = interaction.user.id;
    const canAdjustParticipants = session.state === "registering" ||
    ...
    switch (action) {
        ...
        case "leave":
            ...
            session.captainCandidates = session.captainCandidates.filter((id) => id !== userId);
            await refresh(interaction, session, guild);
            return;
        case "close":
            ...
            session.kickMode = false;
            session.kickSelections = [];
            await continueAsNewMessage(interaction, session, guild, "**팀장 선정**이 아래에서 시작되었습니다.");
            return;
        case "repost":
            if (session.state !== "registering") {
                await interaction.reply({
            ...
            await continueAsNewMessage(interaction, session, guild, "**모집**이 아래에서 계속됩니다.");
            return;
        case "captain":
        ...
        case "confirm_captains":
            ...
            (0, types_1.rollPickOrder)(session);
            startDraft(session);
            await continueAsNewMessage(interaction, session, guild, "**팀원 선택**이 아래에서 시작되었습니다.");
            return;
        case "draft_select":
        ...
        case "draft_pick":
            ...
            }
            {
                ...
                session.remaining = session.remaining.filter((id) => !pickedIds.includes(id));
        ...
        case "kick_confirm":
            ...
            if (session.participants.length - session.kickSelections.length < 2) {
                await interaction.reply({
